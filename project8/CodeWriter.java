...
class CodeWriter {
    ...
    private static final String CMD_POP_STATIC = "// pop static [i]"
        ...
        + LINE_SEPARATOR
        + LINE_SEPARATOR;
    private static final String CMD_PUSH = "// push [i]"
        + LINE_SEPARATOR + "    @[i]"
        + LINE_SEPARATOR + "    D=M"
        + LINE_SEPARATOR + "    @SP"
        + LINE_SEPARATOR + "    A=M"
    ...
    // Branching commands
    private static final String CMD_LABEL = "// label [label]"
        + LINE_SEPARATOR + "([functionName]$[label])"
        + LINE_SEPARATOR
        + LINE_SEPARATOR;
    private static final String CMD_GOTO = "// goto [label]"
        + LINE_SEPARATOR + "    @[functionName]$[label]"
        + LINE_SEPARATOR + "    0;JMP"
        + LINE_SEPARATOR
        + LINE_SEPARATOR;
    private static final String CMD_IF = "// if-goto [label]"
        + LINE_SEPARATOR + "    @SP"
        + LINE_SEPARATOR + "    AM=M-1"
        + LINE_SEPARATOR + "    D=M"
        + LINE_SEPARATOR + "    @[functionName]$[label]"
        + LINE_SEPARATOR + "    D;JNE"
        + LINE_SEPARATOR
        + LINE_SEPARATOR;
    
    // Function commands
    private static final String CMD_FUNCTION = "// function [functionName] [nVars]"
        + LINE_SEPARATOR + "([functionName])"
        + LINE_SEPARATOR
        + LINE_SEPARATOR;
    private static final String CMD_CALL = "// call [function] [nArgs]"
        + LINE_SEPARATOR
        + LINE_SEPARATOR
        + CMD_PUSH_CONSTANT.replace("[i]", "[functionName]$ret.[i]")
        + CMD_PUSH.replace("[i]", "LCL")
        + CMD_PUSH.replace("[i]", "ARG")
        + CMD_PUSH.replace("[i]", "THIS")
        + CMD_PUSH.replace("[i]", "THAT")
        + "// Reposition ARG"
        + LINE_SEPARATOR + "    @5"
        + LINE_SEPARATOR + "    D=A"
        + LINE_SEPARATOR + "    @SP"
        + LINE_SEPARATOR + "    D=M-D"
        + LINE_SEPARATOR + "    @[nArgs]"
        + LINE_SEPARATOR + "    D=D-A"
        + LINE_SEPARATOR + "    @ARG"
        + LINE_SEPARATOR + "    M=D"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + "// Reposition LCL"
        + LINE_SEPARATOR + "    @SP"
        + LINE_SEPARATOR + "    D=M"
        + LINE_SEPARATOR + "    @LCL"
        + LINE_SEPARATOR + "    M=D"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + "// Goto [function]"
        + LINE_SEPARATOR + "    @[function]"
        + LINE_SEPARATOR + "    0;JMP"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + "([functionName]$ret.[i])"
        + LINE_SEPARATOR
        + LINE_SEPARATOR;
    private static final String CMD_RETURN = "// return"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + "// Get return address"
        + LINE_SEPARATOR + "    @5"
        + LINE_SEPARATOR + "    D=A"
        + LINE_SEPARATOR + "    @LCL"
        + LINE_SEPARATOR + "    A=M-D"
        + LINE_SEPARATOR + "    D=M"
        + LINE_SEPARATOR + "    @R14"
        + LINE_SEPARATOR + "    M=D"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + "// Put return value in *ARG"
        + LINE_SEPARATOR + "    @SP"
        + LINE_SEPARATOR + "    A=M-1"
        + LINE_SEPARATOR + "    D=M"
        + LINE_SEPARATOR + "    @ARG"
        ...
        + LINE_SEPARATOR + "    M=D"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + "// Reposition SP to ARG + 1"
        + LINE_SEPARATOR + "    @ARG"
        + LINE_SEPARATOR + "    D=M+1"
        + LINE_SEPARATOR + "    @SP"
        + LINE_SEPARATOR + "    M=D"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + "// Restore THAT"
        + LINE_SEPARATOR + "    @LCL"
        + LINE_SEPARATOR + "    A=M-1"
        + LINE_SEPARATOR + "    D=M"
        + LINE_SEPARATOR + "    @THAT"
        ...
        + LINE_SEPARATOR + "    M=D"
        + LINE_SEPARATOR
        + LINE_SEPARATOR + " // Goto return address"
        + LINE_SEPARATOR + "	@R14"
        + LINE_SEPARATOR + "	A=M"
        + LINE_SEPARATOR + "	0;JMP"
        + LINE_SEPARATOR
    ...
    private static final String CMD_INIT = "// bootstrap code"
        + LINE_SEPARATOR + "    @256"
        + LINE_SEPARATOR + "    D=A"
        + LINE_SEPARATOR + "    @SP"
        + LINE_SEPARATOR + "    M=D"
    ...
    private long symEqIndex;
    private long symGtIndex;
    private long symLtIndex;
    private long symCallIndex;
    private String symFunctionName;

    CodeWriter(String pathname, boolean includeStartupCode) throws Exception {
    ...
    // Necessary for static variable names
    void setFilename(String filename) {
        filenameNoExt = filename.substring(0, filename.lastIndexOf('.'));
    }

    void writeInit() throws Exception {
        writer.write(CMD_INIT, 0, CMD_INIT.length());
    ...
    void writeArithmetic(String command) throws Exception {
        ...
        switch (command) {
            case "eq":
                cmd = cmd.replace("[index]", Long.toString(symEqIndex));
                symEqIndex++;
                break;
            case "gt":
                cmd = cmd.replace("[index]", Long.toString(symGtIndex));
                symGtIndex++;
                break;
            case "lt":
                cmd = cmd.replace("[index]", Long.toString(symLtIndex));
                symLtIndex++;
                break;
        }
    ...
    void writePushPop(CommandType commandType, String segment, int index) throws Exception {
        ...
        switch (segment) {
            ...
            case "static":
                ...
                cmd = cmd.replace("[prefix]", filenameNoExt).replace("[i]", Integer.toString(index));
                break;
            case "temp":
    ...
    void writeFunction(String functionName, int nVars) throws Exception {
        String cmd = CMD_FUNCTION.replace("[functionName]", functionName).replace("[nVars]", Integer.toString(nVars));
        symFunctionName = functionName;
        symCallIndex = 0;
        
        // push 0 to stack nVars times
        for (int i = 0; i < nVars; i++)
            cmd += CMD_PUSH_CONSTANT.replace("[i]", "0");
        
        writer.write(cmd, 0, cmd.length());
    }

    void writeCall(String functionName, int nArgs) throws Exception {
        String cmd = CMD_CALL.replace("[functionName]", symFunctionName).replace("[i]", Long.toString(symCallIndex));
        cmd = cmd.replace("[function]", functionName).replace("[nArgs]", Integer.toString(nArgs));
        symCallIndex++;
    ...
    void writeReturn() throws Exception {
        writer.write(CMD_RETURN, 0, CMD_RETURN.length());
    }

    void writeLabel(String label) throws Exception {
        String cmd = CMD_LABEL.replace("[functionName]", symFunctionName).replace("[label]", label);
        writer.write(cmd, 0, cmd.length());
    }

    void writeGoto(String label) throws Exception {
        String cmd = CMD_GOTO.replace("[functionName]", symFunctionName).replace("[label]", label);
        writer.write(cmd, 0, cmd.length());
    }

    void writeIf(String label) throws Exception {
        String cmd = CMD_IF.replace("[functionName]", symFunctionName).replace("[label]", label);
        writer.write(cmd, 0, cmd.length());
    }
